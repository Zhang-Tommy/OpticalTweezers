...
def compute_pixel(px, py, spots, n_spots, slm_resolution):
    ...
    #jax.debug.print("{totalA}", totalA = totalA)
...
def compute_row(row, spots_params, n_spots, slm_resolution):
    cols = jnp.arange(slm_resolution)
    return jax.vmap(lambda col: compute_pixel(row, col, spots_params, n_spots, slm_resolution))(cols)

@functools.partial(jax.jit, static_argnums=(1,2))
def calculate_phase_mask(spots_params, n_spots, slm_resolution):
    rows = jnp.arange(slm_resolution)
    return jax.vmap(lambda row: compute_row(row, spots_params, n_spots, slm_resolution))(rows)

@functools.partial(jax.jit, static_argnums=(1,2))
def batch_calculate_masks(spots_batch, n_spots, slm_resolution):
    return jax.vmap(functools.partial(calculate_phase_mask, n_spots=n_spots, slm_resolution=slm_resolution))(spots_batch)
...
if __name__ == "__main__":
    ...
    mask = calculate_phase_mask(spots_parameters, 1, 64)
