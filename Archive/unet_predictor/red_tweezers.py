...
slm_resolution = 512 # pixels resolution of SLM (square)
...
def line_trap_logic(amplitude, length, pos, line, k):
    ...
    def true_fn(amplitude, length, pos, line, k):
        ...
        # Use jax.lax.cond again for the inner condition
        return jax.lax.cond(
            sx != 0.0,
...
def adjust_phase(amplitude, phase, totalA):
    # Handle the first condition: if amplitude == 0.0, set phase = 0.0
    phase = jax.lax.cond(
        amplitude == 0.0,
        lambda: 0.0,  # True branch: set phase to 0.0
        lambda: phase,  # False branch: keep phase unchanged
    )

    # Handle the second condition: if totalA > 0.0, scale phase by amplitude / totalA
    phase = jax.lax.cond(
        totalA > 0.0,
        lambda: phase * jnp.clip(amplitude / totalA, 0.0, 1.0),  # True branch: scale phase
        lambda: phase,  # False branch: keep phase unchanged
    )
...
def calculate_totalA(spots):
    """
    Calculate totalA as the sum of all spots[1, 0] elements.
    spots is a (n_spots, 4, 4) tensor.
    """
    return jnp.sum(spots[:, 1, 0])

def calculate_pixel_value(phase, blazing):
    """
    Calculate the pixel value using the blazing table with linear interpolation.
    phase: The phase value for the pixel.
    blazing: The blazing table (1D array of values).
    """
    # Calculate phint and alpha
    phint = jnp.floor((phase / (2 * jnp.pi) + 0.5) * (len(blazing) - 1))
    phint = phint.astype(int)  # Convert to integer index
    alpha = (phase / (2 * jnp.pi) + 0.5) * (len(blazing) - 1) - phint  # Fractional part

    # Ensure phint + 1 does not exceed the blazing table bounds
    phint = jnp.clip(phint, 0, len(blazing) - 2)
    alpha = jnp.clip(alpha, 0.0, 1.0)

    # Linear interpolation between blazing[phint] and blazing[phint + 1]
...
def compute_pixel(px, py, spots, n_spots):
    ...
    #phase = adjust_phase(amplitude, phase, totalA)
