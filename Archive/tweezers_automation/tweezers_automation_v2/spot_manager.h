...
public:
    ...
    void create_donut(int x_pos, int y_pos, int vortex_charge, int z);
    void create_line(int x_pos, int y_pos, int x_len, int y_len);
    void move_trap(int x_trap, int y_trap, int x_new, int y_new, bool update);
    void translate_trap(float x_trap, float y_trap, float x_new, float y_new, float um_sec);
    const std::vector<std::pair<int, int>> interpolate_path(float x_trap, float y_trap, float x_new, float y_new);
