...
void autoCanny(cv::Mat image, cv::Mat& output, float sigma) {
...
int main1(int argc, char** argv)
{
	...
	if (argc < 2)
	{
		cout << " Usage: detect.exe ImageToLoadAndDisplay NumberOfLoops Debug" << endl;
		return -1;
	}
	if (argc > 2) {
		repeat = 1;
	...
	cv::Mat oriimg;
	oriimg = cv::imread(argv[1], cv::IMREAD_GRAYSCALE); // Read the file
	if (oriimg.empty()) // Check for invalid input
	{
		cout << "Could not open or find the image" << endl;
		return -1;
	...
	cv::subtract(oriimg, b, img1, cv::noArray(), CV_64FC1);
	cv::divide(img1, a, img2);
	if (Debug) cv::imwrite("1 removeSmudges.jpg", img2);
	COUT("removed smudges jpeg");
	//Step 1: detecting beads
	img2.convertTo(img, CV_8UC1);
	cv::Mat Step1;
	int offset = 2;
	...
	cv::drawKeypoints(img, keypoints, imgWithKeypoints, cv::Scalar(255, 255, 0), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
	...
	for (int i = 0; i < contours.size(); i++) {
		if (areas[i] > maxContourArea / 8 && areas[i] / cv::arcLength(contours[i], true) > 3) {
			cv::drawContours(Step4, contours, i, cv::Scalar(0, 0, 255), 2);  // red outline 
			cv::RotatedRect  rRect = cv::minAreaRect(contours[i]);
			cv::Point2f vertices[4];
			rRect.points(vertices);
			for (int i = 0; i < 4; i++) {
				cv::line(Step4, vertices[i], vertices[(i + 1) % 4], cv::Scalar(255, 0, 0));  // blue rectangle around red outline
			}
		}
	}
	for (int i = 0; i < contours1.size(); i++) {
		if (areas1[i] > maxContourArea / 20 && areas1[i] / cv::arcLength(contours1[i], true) > 3) {
			...
			rRect.points(vertices);
			for (int i = 0; i < 4; i++) {
				cv::line(Step4, vertices[i], vertices[(i + 1) % 4], cv::Scalar(255, 255, 0)); // 255 255 0
			}
		}
	}
